...
--      table's own record, not a partial part of a combined key.
-- 3NF: Account, about, contact, and portfolio section data are separated so
--      non-key details like phone, bio, and project title do not depend on each other.
...
-- Table: education
-- 2NF example: degree, institution, dates, description, and result all describe
-- one education record identified by id and owned by user_id.
...
-- Table: skills
-- 1NF example: every skill is one row; skills are not stored as one comma-separated
-- field inside the users table.
...
-- Table: projects
-- 3NF example: project-specific details stay in projects instead of repeating
-- user email, phone, or biography data here.
